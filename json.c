...
static bool_ty json_parse_value(struct json_parser_st* parser, struct json_value_st* json_value);
static bool_ty json_parse_value_null(struct json_parser_st* parser, struct json_value_st* json_value);
static bool_ty json_parse_value_bool(struct json_parser_st* parser, struct json_value_st* json_value);
static bool_ty json_parse_value_object(struct json_parser_st* parser, struct json_value_st* json_value);
static bool_ty json_parse_value_array(struct json_parser_st* parser, struct json_value_st* json_value);
static bool_ty json_parse_value_string(struct json_parser_st* parser, struct json_value_st* json_value);
static bool_ty json_parse_value_number(struct json_parser_st* parser, struct json_value_st* json_value);
static bool_ty json_parse_string(struct json_parser_st* parser, char** string);
...
static struct hash_table_bucket_element_st* hash_table_bucket_find(const struct hash_table_bucket_st* hash_table_bucket, const char* key);
static struct hash_table_bucket_st*         json_object_table_find_bucket(const struct json_value_st* json_value, const char* key);
static bool_ty                              json_object_table_resize(struct json_value_st* json_value, size_ty new_num_buckets);
...
struct json_value_st* json_parse(const char* json_source, size_ty json_source_length) {
    ...
    if (root == NULL) {
        log_error("JSON parser error: Failed to allocate memory for JSON root.\n");
        return NULL;
    }
...
bool_ty json_bool(const struct json_value_st* json_value) {
    return json_value->type == JSON_VALUE_TYPE_BOOL_TRUE ? b_TRUE : b_FALSE;
}
...
struct json_value_st* json_object_add(struct json_value_st* json_value, const char* key) {
    ...
    if (element == NULL) {
        log_error("JSON parser error: Failed to allocate memory for JSON Object hash table bucket element.\n");
        return NULL;
    }
...
bool_ty json_array_set_capacity(struct json_value_st* json_value, size_ty new_capacity) {
    ...
    if (new_data == NULL) {
        log_error("JSON parser error: Failed to reallocate memory for JSON Array.\n");
        return b_FALSE;
    }
...
void json_string_set(struct json_value_st* json_value, const char* string_to_copy) {
    if (json_is_string(json_value)) {
        ...
        if (json_value->value.as_string == NULL)
            log_error("JSON parser error: Failed to allocate memory for JSON string.\n");
    }
}
...
bool_ty json_parse_value(struct json_parser_st* parser, struct json_value_st* json_value) {
    if (json_parse_value_object(parser, json_value) ||
        ...
        return b_TRUE;
    log_error("JSON parser error: Unexpected token '%c'.\n", *parser->last_char);
    return b_FALSE;
}
        ...
        if (*parser->last_char == ':') {
            ...
            if (json_parse_value(parser, &property_value)) {
                ...
                if (new_object_property != NULL) {
                    ...
                    log_error("JSON parser error: Failed to add property \"%s\" to object.\n", key);
            } else {
                free(key);
                break;
            }
            ...
            log_error("JSON parser error: Expected ':' after property name.\n");
            free(key);
            break;
        }
    }
...
bool_ty json_parse_value_array(struct json_parser_st* parser, struct json_value_st* json_value) {
    ...
    for (;;) {
        ...
        if (new_array_item == NULL)
            log_error("JSON parser error: Failed to add element to array.\n");
        else 
...
bool_ty json_parse_value_number(struct json_parser_st* parser, struct json_value_st* json_value) {
    ...
    if (errptr == parser->last_char) {
        log_error("JSON parser error: Failed to parse JSON number.\n");
        return b_FALSE;
    }
...
bool_ty json_parse_string(struct json_parser_st* parser, char** string) {
        ...
        if (*parser->last_char == '"') {
            ...
            if (temp_string_length > 0) {
                *string = malloc(sizeof(char) * (temp_string_length + 1));
                if (*string == NULL) {
                    log_error("JSON parser error: Failed to allocate memory for parsed JSON string.\n");
                    return b_FALSE;
                }
                ...
                case 'u':
                    for (int i = 0; i < 4; ++i) {
                        ...
                        if (!((*parser->last_char >= '0' && *parser->last_char <= '9') || (*parser->last_char >= 'a' && *parser->last_char <= 'f') || (*parser->last_char >= 'A' && *parser->last_char <= 'F'))) {
                            log_error("JSON parser error: Bad unicode escape.\n");
                            return b_FALSE;
                        }
                    }
                    break;
    ...
    log_error("JSON parser error: Failed to parse string: Unterminated string.\"\n");
    return b_FALSE;
}
...
bool_ty json_object_table_resize(struct json_value_st* json_value, size_ty new_num_buckets) {
    ...
    if (new_hash_table_buckets == NULL) {
        log_error("JSON parser error: Failed to allocate memory for JSON Object hash table buckets.\n");
        return b_FALSE;
    }
